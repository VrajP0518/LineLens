...
def main() -> int:
    ...
    mlb_model_picks = mlb_predictions.get("model_picks", [])
    ...
    check(all(row.get("sport") == "MLB" for row in mlb_model_picks), "MLB model picks are sport-labeled")
    ...
    model_only = [row for row in mlb_candidates if row.get("model_only") is True]
    check(all(row.get("line") is None and row.get("original_price") is None and row.get("probability") is None and row.get("edge") is None for row in model_only), "MLB model-only projections never infer market fields")
    check(all(str(row.get("player_name") or "").strip() for row in model_only), "MLB model-only projections have resolved player names")
    check(all(row.get("model_only") is not True for row in mlb_rows), "MLB model-only projections cannot be published")
    check(len({row.get("prediction_id") for row in mlb_model_picks}) == len(mlb_model_picks), "MLB model pick IDs are unique")
    check(all(row.get("model_only_pick") is True and row.get("candidate_only") is False for row in mlb_model_picks), "MLB model picks have an explicit internal-pick boundary")
    check(all(row.get("line") is None and row.get("original_price") is None and row.get("edge") is None for row in mlb_model_picks), "MLB model picks never claim bookmaker fields")
    check(all(0 <= float(row.get("probability")) <= 1 and row.get("model_threshold") is not None for row in mlb_model_picks), "MLB model picks have bounded model probabilities and thresholds")
    check(not ({row.get("prediction_id") for row in mlb_model_picks} & {row.get("prediction_id") for row in mlb_candidates}), "MLB model picks and candidates do not overlap")
    if mlb_predictions.get("metadata", {}).get("status") == "model_only_projections":
    ...
    mlb_export = (ROOT / "src/mlb/export_prop_predictions.py").read_text(encoding="utf-8")
    wnba_export = (ROOT / "src/wnba/export_prop_predictions.py").read_text(encoding="utf-8")
    check("production_model_ready" in mlb_export and "metadata.production_ready" in mlb_export, "MLB props require a selected production model registry entry")
    check("production_model_ready" in wnba_export and "metadata.production_ready" in wnba_export, "WNBA props require a selected production model registry entry")
    check(all(row.get("model_validation_status") in {"production_ready", "research_only", None} for row in mlb_rows + mlb_candidates + rows + candidates), "prop rows expose a bounded model validation state")
