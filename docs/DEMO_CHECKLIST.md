...
After downloading `LineLens-Sports-Windows-v6.0.0` from GitHub Actions, verify: fresh install/open; first launch without Python, `.env`, network, or odds credentials; Home bundled data; Underdogs real-odds qualification; MLB date selection and score/status consistency; Soccer empty/live state; every unique Models entry; GameCast scoring and player tables; widget; responsive navigation; favorites persistence; ticker clearance; Presentation Mode; approved-model update status; and successful reopen after closing. Review the uploaded 125% and 150% DPI screenshots from the Windows DPI Visual Audit workflow.
